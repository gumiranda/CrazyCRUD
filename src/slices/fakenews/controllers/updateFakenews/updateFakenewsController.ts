...
import { UpdateFakenews } from "@/slices/fakenews/useCases";

export class UpdateFakenewsController extends Controller {
  constructor(
    private readonly validationQuery: Validation,
    private readonly validationBody: Validation,
    private readonly updateFakenews: UpdateFakenews
  ) {
    super();
  }
  async execute(httpRequest: HttpRequest<any>): Promise<HttpResponse<any>> {
    ...
    const fakenewsUpdated = await this.updateFakenews(
      {
        fields: {
          ...httpRequest?.query,
